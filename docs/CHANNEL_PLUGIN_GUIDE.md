...
Build a custom PgFlow channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of PgFlow (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.
...
```
pgflow-channel-webhook/
├── pgflow_channel_webhook/
...
```python
# pgflow_channel_webhook/__init__.py
from pgflow_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# pgflow_channel_webhook/channel.py
import asyncio
...
```bash
pip install -e .
pgflow gateway   # the webhook channel will be discovered automatically
```

Edit `~/.pgflow/config.json`:
...
```bash
pgflow channels login <channel_name>
pgflow channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict. Override to declare your fields. |
| `transcribe_audio(file_path)` | Transcribes audio via Groq Whisper (if configured). |
| `supports_streaming` (property) | `True` when config has `"streaming": true` **and** subclass overrides `send_delta()`. |
| `is_running` | Returns `self._running`. |
| `login(force=False)` | Perform interactive login. Returns `True` if already authenticated or login succeeds. |

### Optional (streaming)

| Method | Description |
|--------|-------------|
| `async send_delta(chat_id, delta, metadata?)` | Override to receive streaming chunks. |
...
Channels can opt into real-time streaming — the agent sends content token-by-token instead of one final message.
...
Override `default_config()` so `pgflow onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `pgflow-channel-{name}` | `pgflow-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `pgflow_channel_{name}` | `pgflow_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/pgflow-channel-webhook
cd pgflow-channel-webhook
pip install -e .
pgflow gateway   # test end-to-end
```
