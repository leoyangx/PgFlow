...
import pgflow.channels.matrix as matrix_module
from pgflow.bus.events import OutboundMessage
from pgflow.bus.queue import MessageBus
from pgflow.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from pgflow.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("pgflow.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "pgflow.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("pgflow.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "pgflow.channels.matrix.asyncio.create_task", _fake_create_task
    )
