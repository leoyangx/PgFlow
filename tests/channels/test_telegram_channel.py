...
from pgflow.channels.telegram import TELEGRAM_REPLY_CONTEXT_MAX_LEN, TelegramChannel
from pgflow.channels.telegram import TelegramConfig
...
class _FakeBot:
    ...
    async def get_me(self):
        self.get_me_calls += 1
        return SimpleNamespace(id=999, username="pgflow_test")
...
async def test_start_creates_separate_pools_with_proxy(monkeypatch) -> None:
    ...
    monkeypatch.setattr("pgflow.channels.telegram.HTTPXRequest", _FakeHTTPXRequest)
    monkeypatch.setattr(
        "pgflow.channels.telegram.Application",
...
async def test_send_text_retries_on_timeout() -> None:
    ...
    import pgflow.channels.telegram as tg_mod
...
async def test_send_remote_media_url_after_security_validation(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr("pgflow.channels.telegram.validate_url_target", lambda url: (True, ""))
...
async def test_send_blocks_unsafe_remote_media_url(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr(
        "pgflow.channels.telegram.validate_url_target",
...
async def test_group_policy_mention_accepts_text_mention_and_caches_bot_identity() -> None:
    ...
    mention = SimpleNamespace(type="mention", offset=0, length=12)
    await channel._on_message(_make_telegram_update(text="@pgflow_test hi", entities=[mention]), None)
    await channel._on_message(_make_telegram_update(text="@pgflow_test again", entities=[mention]), None)
...
async def test_group_policy_mention_accepts_caption_mention() -> None:
    ...
    await channel._on_message(
        _make_telegram_update(caption="@pgflow_test photo", caption_entities=[mention]),
        None,
    )

    assert len(handled) == 1
    assert handled[0]["content"] == "@pgflow_test photo"
...
async def test_download_message_media_returns_path_when_download_succeeds(
    ...
    monkeypatch.setattr(
        "pgflow.channels.telegram.get_media_dir",
