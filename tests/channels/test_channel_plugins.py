...
from pgflow.config.schema import ChannelsConfig
...
def test_discover_plugins_loads_entry_points():
    from pgflow.channels.registry import discover_plugins
...
async def test_manager_loads_plugin_from_dict_config():
    ...
    with patch(
        "pgflow.channels.registry.discover_all",
...
def test_channels_login_uses_discovered_plugin_class(monkeypatch):
    from pgflow.cli.commands import app
    ...
    monkeypatch.setattr("pgflow.config.loader.load_config", lambda: Config())
    monkeypatch.setattr(
        "pgflow.channels.registry.discover_all",
...
def test_builtin_channel_default_config():
    ...
    from pgflow.channels.telegram import TelegramChannel
