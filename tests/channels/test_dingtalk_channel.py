...
try:
    from pgflow.channels import dingtalk
...
import pgflow.channels.dingtalk as dingtalk_module
from pgflow.channels.dingtalk import DingTalkChannel, PgFlowDingTalkHandler
from pgflow.channels.dingtalk import DingTalkConfig
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = PgFlowDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/pgflow_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/pgflow_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "pgflow.config.paths.get_media_dir",
