...
from pgflow.providers.base import LLMResponse
...
def _make_loop():
    ...
    with patch("pgflow.agent.loop.ContextBuilder"), \
         ...
         patch("pgflow.agent.loop.SubagentManager"):
...
class TestRestartCommand:
    ...
    async def test_restart_sends_message_and_calls_execv(self):
        from pgflow.command.builtin import cmd_restart
        from pgflow.command.router import CommandContext

        loop, bus = _make_loop()
        msg = InboundMessage(channel="cli", sender_id="user", chat_id="direct", content="/restart")
        ctx = CommandContext(msg=msg, session=None, key=msg.session_key, raw="/restart", loop=loop)

        with patch("pgflow.command.builtin.os.execv") as mock_execv:
    ...
    async def test_restart_intercepted_in_run_loop(self):
        ...
        with patch.object(loop, "_dispatch", new_callable=AsyncMock) as mock_dispatch, \
             patch("pgflow.command.builtin.os.execv"):
    ...
    async def test_status_intercepted_in_run_loop(self):
        ...
        with patch.object(loop, "_dispatch", new_callable=AsyncMock) as mock_dispatch:
            ...
            mock_dispatch.assert_not_called()
            out = await asyncio.wait_for(bus.consume_outbound(), timeout=1.0)
            assert "pgflow" in out.content.lower() or "Model" in out.content
