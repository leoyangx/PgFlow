...
from pgflow.agent.tools.web import WebSearchTool
from pgflow.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    async def mock_get(self, url, **kw):
        ...
        return _response(json={
            "web": {"results": [{"title": "PgFlow", "url": "https://example.com", "description": "AI assistant"}]}
        })

    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="brave", api_key="brave-key")
    result = await tool.execute(query="pgflow", count=1)
    assert "PgFlow" in result
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("pgflow.agent.tools.web.DDGS", MockDDGS, raising=False)
    import pgflow.agent.tools.web as web_mod
