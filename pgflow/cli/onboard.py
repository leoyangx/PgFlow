"""Interactive onboarding questionnaire for PgFlow."""
...
def _show_main_menu_header() -> None:
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]PgFlow[{__version__}][/bold cyan]")
    )
    console.print()
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from pgflow.config.schema to get
    ...
    from pgflow.config.schema import AgentDefaults
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"pgflow.channels.{name}")
...
def _prompt_workspace(current_workspace: str) -> str | None:
    """Ask the user where to store PgFlow data. Returns chosen path or None to keep current."""
    from pathlib import Path

    default_path = str(Path.home() / ".pgflow" / "workspace")
...
def run_onboard(initial_config: Config | None = None) -> OnboardResult:
    ...
    # Step 0: Workspace selection — always show when config file doesn't exist yet
    is_first_run = not get_config_path().exists()
    if is_first_run:
        console.clear()
        _show_main_menu_header()
        workspace_path = _prompt_workspace(config.agents.defaults.workspace)
